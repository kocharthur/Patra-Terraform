...
cd /var/www/html
sudo wget https://s3-terraform-bucket-patra.s3.us-east-2.amazonaws.com/itmeme/index-image.html 
sudo wget https://s3-terraform-bucket-patra.s3.us-east-2.amazonaws.com/itmeme/istockphoto-1124656717-612x612.jpeg
...
yum install -y httpd php git
cd /var/www/html
sudo wget https://s3-terraform-bucket-patra.s3.us-east-2.amazonaws.com/video/index-animation.html
sudo wget https://s3-terraform-bucket-patra.s3.us-east-2.amazonaws.com/video/giphy.gif
...
resource "aws_cloudwatch_metric_alarm" "web_cpu_alarm_up_image" {
  alarm_name          = "web_cpu_alarm_up_image1"
...
resource "aws_cloudwatch_metric_alarm" "web_cpu_alarm_down_image" {
  alarm_name          = "web_cpu_alarm_down_image1"
...
resource "aws_cloudwatch_metric_alarm" "web_cpu_alarm_up_video" {
  alarm_name          = "web_cpu_alarm_up_video1"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "CPUUtilization"
  namespace           = "AWS/EC2"
...
resource "aws_cloudwatch_metric_alarm" "web_cpu_alarm_down_video" {
  alarm_name          = "web_cpu_alarm_down_video1"
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = "CPUUtilization"
  namespace           = "AWS/EC2"
